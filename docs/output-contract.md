...
A value naming something on the server (a field, a user, a transition, an issue
type, a transition's resolution) is resolved against the site before the request
is built, never sent for Jira to reject. The refusal carries the candidates,
because an error that only says "unknown" leaves the caller to go and read a
catalogue to find their typo. Not every named value is resolved yet: a priority,
and a value `--field` sends by name, still go as typed, and Jira refuses a bad
one with `BAD_REQUEST`.
...
| `UNKNOWN_RESOLUTION`   | 2    | `issue move --resolution` named a resolution the transition's screen does not offer. `detail` lists every one it does, each with its id, and either can be passed. The set is the screen's and not the site's, because a resolution the site has is not necessarily one the transition offers. The match is by id, then by name in any case, and what is sent is the site's own spelling, because Jira matches the name case-sensitively. |
| `AMBIGUOUS_RESOLUTION` | 2    | Two resolutions the screen offers share that name in some case. `detail` lists both; pass the id. |
...
| `DESTINATION_EXISTS`       | 7    | A download, or `skill --dir`, would replace a file that is already there. It refuses rather than overwriting, because a download that silently replaced a file is indistinguishable from one that worked, and the file it replaced is not recoverable. `--force` allows it.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `TRANSITION_TAKES_NO_RESOLUTION` | 2 | `--resolution` was given for a transition whose screen has no resolution field. Jira refuses this too, on both deployments ("Field 'resolution' cannot be set. It is not on the appropriate screen, or unknown."), so this is its verdict without the round trip, and a `--dry-run` reaches it instead of printing a request Jira would refuse. Read from the field list `expand=transitions.fields` already returns, so it costs no request. None of the default workflows measured on either deployment puts a screen on any transition, so there it is the answer for every one. `jr meta transitions <key> --format xml` shows which transitions carry the field. |
| `TRANSITION_TAKES_NO_COMMENT` | 2 | `--comment` was given for a transition that has no screen, or, on Cloud, whose screen lists no comment field. Jira does not refuse a comment sent with a transition that has no screen: both deployments take the transition and discard the comment, answering 204, so the caller is told a comment was added that does not exist. Refused before the transition is sent, from the field list `expand=transitions.fields` already returns. Data Center never says whether there is a screen and never lists a comment among a screen's fields, so there a transition that lists any field has a screen and takes the comment (measured on 10.4.0); Cloud says, and its screens are still held to listing one, because none has been measured. `jr meta transitions <key> --format xml` shows each transition's fields. |
...
| `SPRINT_CLOSED`            | 7    | `sprint add` into a sprint that is closed. Unlike its two siblings this is diagnosed after the refusal rather than checked before it: the command needs nothing from the sprint when the move succeeds, so a pre-check would put a request on every successful move to describe a failure that usually does not happen. The state is read from the sprint and never from Jira's message, which is a localised sentence rather than a fact. A refusal this cannot confirm stays `BAD_REQUEST`, with Jira's own words in `detail`. |
