...
Two destinations cover the field: `~/.agents/skills` is the cross-agent
convention, read by Codex, Cursor, Gemini CLI and most other skill loaders,
and `~/.claude/skills` is where Claude Code looks. Each recipe below installs
into both.

**Link it, if you installed with Homebrew.** The formula writes the skill with
`jr skill --dir` from the binary it installs, so it always describes that
binary. Homebrew does not write into your home directory, so the links are
yours to make, once; every `brew upgrade` then moves the skill with the binary:
...
If either destination is already a directory, from `--dir` or a copy, remove
it first.
...
```console
$ mkdir -p ~/.agents/skills ~/.claude/skills
$ ln -s "$PWD/skills/jr" ~/.agents/skills/jr        # personal, every project
$ ln -s "$PWD/skills/jr" ~/.claude/skills/jr        # personal, for Claude Code
$ ln -s "$PWD/skills/jr" .agents/skills/jr          # this project only
```
...
```console
$ jr skill --dir ~/.agents/skills/jr        # personal, every project
$ jr skill --dir ~/.claude/skills/jr        # personal, for Claude Code
$ jr skill --dir .agents/skills/jr          # this project only
```
