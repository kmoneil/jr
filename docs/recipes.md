...
binary. Homebrew does not write into your home directory, so the link is yours
to make, once; every `brew upgrade` then moves the skill with the binary:
...
If `~/.claude/skills/jr` is already a directory, from `--dir` or a copy, remove
it first.
